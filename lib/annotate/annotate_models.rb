...
module AnnotateModels
  ...
  class << self
    ...
    def get_schema_info(klass, header, options = {}) # rubocop:disable Metrics/MethodLength
      ...
      if options[:show_referenced_by] && klass.table_exists?
        info << get_referenced_by_info(klass, options)
      end
    ...
    def get_referenced_by_info(klass, options = {})
      return '' unless options[:show_referenced_by]
      return '' unless klass.connection.table_exists?

      db_name = klass.connection.current_database
      current_table = klass.table_name

      results = klass.connection.exec_query(<<~SQL)
    SELECT TABLE_NAME, COLUMN_NAME
    FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE
    WHERE
      REFERENCED_TABLE_NAME = '#{current_table}'
      ...
      max_size = results.map { |r| "#{r["TABLE_NAME"]}.#{r["COLUMN_NAME"]}".size }.max + 2

      results.each do |r|
        line = sprintf("#  %-#{max_size}s => %s.id", "#{r["TABLE_NAME"]}.#{r["COLUMN_NAME"]}", current_table)
        info << line.rstrip + "\n"
      end
